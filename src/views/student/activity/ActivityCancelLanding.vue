<template>
  <div class="cancel-landing">
    <h2>取消报名</h2>
    <p v-if="!activityId" class="hint error">
      缺少参数 <code>activityId</code>（或 <code>id</code>）。请从我的活动进入，或使用完整链接。
    </p>
    <p v-else-if="pending" class="hint">正在取消报名…</p>
...
import { cancelActivityRegistration } from '@/api/activity.js'
...
async function runCancel() {
  ...
  try {
    ...
    resultOk.value = ok
    /** 后端 R<String> 多为提示文案；activityId 在请求 Query 中，响应体通常不含回传 ID */
    resultMsg.value =
      msg ||
      (ok ? '取消成功' : '取消失败') ||
    ...
    if (ok) {
      ...
      router.replace({ path: '/student/activity/my' })
    } else {
